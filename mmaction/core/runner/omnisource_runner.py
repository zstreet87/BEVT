...
class MultiSourceRunner(EpochBasedRunner):
    """OmniSource Epoch-based Runner.
    ...
    def run_iter(self, data_batch, train_mode, source, data_type, **kwargs):
        if self.batch_processor is not None:
            outputs = self.batch_processor(
                self.model, data_batch, train_mode=train_mode, **kwargs)
        elif train_mode:
            outputs = self.model.train_step(data_batch, self.optimizer, data_type=data_type,
                                            **kwargs)
        else:
            outputs = self.model.val_step(data_batch, self.optimizer, **kwargs)
        if not isinstance(outputs, dict):
    ...
    def train(self, data_loaders, **kwargs):
        ...
        import os
        max_steps = int(os.environ.setdefault('BEVT_MAX_STEPS', '1200'))
        early_stop = max_steps != 0
        for i, data_batch in enumerate(self.main_loader):
            self._inner_iter = i
            data_batch_list = [data_batch]
            for idx in range(len(self.aux_iters)):
                aux_data_batch = next(self.aux_iters[idx])
                data_batch_list.append(aux_data_batch)

            self.call_hook('before_train_iter')
            self.run_iter(data_batch_list, train_mode=True, source='', data_type=data_type)
            self.call_hook('after_train_iter')
